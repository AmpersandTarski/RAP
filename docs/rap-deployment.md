...
The following overview outlines the workflow for creating a prototype as a student. The student will first log in to the RAP website, create an Ampersand script, compile it, and then generate the prototype based on the script.
...
The deployment file pulls the `ampersandtarski/ampersand-rap:<<version>>` image from the ampersandtarski repository on docker hub, and uses configmaps as environment variables. The RAP pod connects to the database, using the information in the Secret `db-secrets`, which is generated by Kustomize upon deployment. `db-secrets` contains the root password for the MYSQL database, the MYSQL user, and password. The mysql rap4-db is accessed with these values. Configuration data such as the server URL, Ampersand user and log configuration is included as environmental variable in the container specifications.

The service and ingress rule are used to reach the RAP application from an external entry point. The ClusterIP service exposes the pod to a web server, and the ingress routes incoming requests to the web-server service based on the path.
...
The deployment manifest file pulls the  `ampersandtarski/rap4-student-prototype:<<version>>` image from the ampersandtarski repository on docker hub. The student Ampersand script that is referred to in the workflow is added to a zip file, and this zip file is then added as the first base64-encoded argument to the manifest file, the name of the script from where the ampersand compiler should start is added as the second base64-encoded argument. Once the pod starts, it first runs the run-student-prototype bash script. This script decodes the base64-encoded inputs, unzips the file(s) and generates the prototype with the Ampersand compiler.
...
The service and ingress rule are used to reach the student123 application from an external entry point. The ClusterIP service exposes the pod to a web server, and the ingress routes incoming requests to the web-server service based on the path.
...
Part of the deployment is printed below. When a student prototype is generated, the manifest file is copied to a unique student folder. Various variables are replaced with student specific values:

| Variable          | Description                                                            |
| - | - |
| {{student}}       | Student user name                                                      |
| {{namespace}}     | Namespace of the Kubernetes Cluster (rap-dev, rap-staging, rap-production) |
| {{containerImage}} | The student prototype image to use |
| {{dbName}} | Name of the db to connect to |
| {{dbSecrets}} | Name of the Secret resource |
| {{zipContent}} | Base64 encoded zip containing the adl scripts                                      |
| {{mainAdl}} | Base64 encoded name of adl file |
| {{hostName}} | Host name of parent |
| {{tlsSecret}} | Secret name used for certificate generation |
...
```
echo \"<<base64 encoded zip>> <<base64 encode mainAdl>>\" | docker run",
            [ "--name \"<<student name>>\"",
            ...
            "--label traefik.http.routers.<<student name>>-insecure.rule=\"Host(\\`student name.<<host name>>\\`)\"", // e.g. student123.rap.cs.ou.nl
            ...
            "-e AMPERSAND_DBNAME=\"student_<<student name>>\"",
            ...
            "-e AMPERSAND_SERVER_URL=\"https://<<student name>>.<<host name>>\"",
...
The student prototype container is started by calling the Docker run command in the code. The content of the `zipContentForCommandline` variable, which refers to the "enrollment" script in the workflow, is base64 encoded and written to the Docker stdin. When the container starts, it runs the **run-student-prototype&#46;sh** script, which unzips the file and these files are then used to generate the Ampersand prototype.

In Kubernetes, the student prototype container starts when the student123 manifest file is applied. The **running-student-prototype2&#46;sh** script is used in Kubernetes, which is adjusted for Kubernetes and refers to the new script. The command field, which specifies the entry point of the pod, is set to `["bash", "/run-student-prototype.sh"]`, while the args field refers to the "enrollment" script in the workflow, which is also base64 encoded. The first argument of the Kubernetes container is unzipepd, and the unzipped files are then used to generate the Ampersand prototype.
