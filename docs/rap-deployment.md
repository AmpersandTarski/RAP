# Deployment of RAP
This chapter helps software engineers to understand the different ways to deploy RAP:
* on a docker platform for multiple users
* on a kubernetes platform  for multiple users
* on a private laptop for personal use

Please find separate instructions for:
* [deploying, testing, and maintaining RAP on a docker server](./README.md);
* [preparing a Kubernetes environment on Azure](preparing-azure.md)
.
...
The typical reason to (re-)deploy RAP is to provide this service to your students.

The RAP application is an information system generated by Ampersand. Its architecture consists of a backend connected to [a database, an API and a frontend application](https://ampersandtarski.github.io/ampersand/architecture-of-an-ampersand-application## "Explain this architecture"), like any other application Ampersand generates.

## Platform independent overview
To understand the deployment of RAP, let us look at the different components involved.
